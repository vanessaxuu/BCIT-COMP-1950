<section class="py-5">
        <div class="container">
<!-- Material form contact -->
<form>
    <h1 class="h4 text-center mb-4">Quiz #1</h1>

        <div class="form-row">
            <div class="col">
            <input type="text" class="form-control" placeholder="First name">
            </div>
            <div class="col">
            <input type="text" class="form-control" placeholder="Last name">
            </div>
        </div>

    <div class="form-inline">
        <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Name one brand of HTTP server (1 mark):</label>
            <select class="custom-select my-1 mr-sm-2" id="inlineFormCustomSelectPref">
                <option selected>Choose...</option>
                <option value="1">WAMP</option>
                <option value="2">APACHE</option>
                <option value="3">FILEZILLA</option>
            </select>
    </div>
        <div class="form-group">
            <label for="exampleFormControlTextarea1">Describe one advantage of implementing an error 404 page (2 marks):</label>
            <textarea class="form-control" id="exampleFormControlTextarea1" rows="3"></textarea>
        </div>  

        <div class="form-group">
            <label for="exampleFormControlSelect1">The filename of configuring an Apache server is (1 mark):</label>
            <select class="form-control" id="exampleFormControlSelect1">
            <option>apchie.ini</option>
            <option>apache.conf</option>
            <option>html.conf</option>
            <option>.htaccess</option>
            </select>
        </div>

        <label>In server-side scripting the conditional execution of code can be accomplished using (1 mark):</label>
        <div class="form-check">
            <input class="form-check-input" type="radio" name="exampleRadios" id="exampleRadios1" value="option1" checked>
            <label class="form-check-label" for="exampleRadios1">
                an if statement
            </label>
        </div>

        <div class="form-check">
            <input class="form-check-input" type="radio" name="exampleRadios" id="exampleRadios2" value="option2">
            <label class="form-check-label" for="exampleRadios2">
                a choice tree
            </label>
        </div>

        <div class="form-check disabled">
            <input class="form-check-input" type="radio" name="exampleRadios" id="exampleRadios3" value="option3">
            <label class="form-check-label" for="exampleRadios3">
                forked code
            </label>
        </div>
                
    <button type="submit" class="btn btn-primary my-1">Submit</button>
</form>
<!-- Material form contact -->
</div>
      </section>